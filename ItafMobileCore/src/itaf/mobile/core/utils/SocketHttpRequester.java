...
public class SocketHttpRequester {
	...
	public static boolean uploadFile(String path, Map<String, String> params,
		...
		try {
			...
			FileBody bin = new FileBody(file.getFile());

			reqEntity.addPart("attachMentFiles", bin);
			for (Iterator<Entry<String, String>> iterator = params.entrySet()
					.iterator(); iterator.hasNext();) {
				Entry<String, String> param = (Entry<String, String>) iterator
						.next();

				StringBody comment = new StringBody(param.getValue());
				reqEntity.addPart(param.getKey(), comment);
			...
			Log.d("SocketHttpRequester",
					"----------------------------------------");
			// Log.d("SocketHttpRequester",response.getStatusLine().toString());
			if (resEntity != null) {
				Log.d("SocketHttpRequester", "Response content length: "
						+ resEntity.getContentLength());
				Log.d("SocketHttpRequester",
						"Chunked?: " + resEntity.isChunked());
			}
